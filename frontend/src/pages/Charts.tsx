...
export default function Charts() {
  ...
  useEffect(() => {
    const fetchLocoData = async () => {
      try {
        const response = await fetch("https://backend-of-railnova.vercel.app/analysis/locos");
  ...
  useEffect(() => {
    const fetchWagonData = async () => {
      try {
        const response = await fetch("https://backend-of-railnova.vercel.app/api/wagon-summary");
        if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
